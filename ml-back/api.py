...
class APIServer(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if parsed_path.path == "/get_task":
            ...
            if (
                "new_material" in query_params
                and "pages" in query_params
                and "time" in query_params
            ):
                try:
                    ...
                    self.end_headers()
                    self.wfile.write(
                        b"Invalid parameters. Please provide valid numbers."
                    )
            else:
                self.send_response(400)
                self.end_headers()
                self.wfile.write(
                    b"Missing parameters. Please provide new_material, pages, and time."
                )
        elif parsed_path.path == "/get_score":
            query_params = parse_qs(parsed_path.query)
            if (
                "Q1" in query_params
                and "Q2" in query_params
                and "weights" in query_params
            ):
                try:
            ...
            query_params = parse_qs(parsed_path.query)
            if (
                "weight" in query_params
                and "weights" in query_params
                and "rate" in query_params
                and "daysLeft" in query_params
            ):
                try:
            ...
            query_params = parse_qs(parsed_path.query)
            if (
                "count_task" in query_params
                and "count_todos" in query_params
                and "count_habits" in query_params
                and "missed_events" in query_params
                and "events" in query_params
                and "reviewed" in query_params
            ):
                try:
